(function(){
  if(window.__appleTouchPatchV40)return;
  window.__appleTouchPatchV40=true;
  ...
  // Ensure modern viewport behavior even if the giant legacy index does not declare every iOS option.
  ...
  style.textContent=`
    ...
    html.apple-touch-device button,html.apple-touch-device a,html.apple-touch-device input,html.apple-touch-device select,html.apple-touch-device textarea,
    html.apple-touch-device [role="button"]{touch-action:manipulation;-webkit-tap-highlight-color:transparent;}
    ...
    html.apple-touch-device .leaflet-control-container .leaflet-control,
    html.apple-touch-device .leaflet-control-container button,
    html.apple-touch-device .leaflet-control-container a{pointer-events:auto!important;}
    html.apple-touch-device .live-signal-marker,
    html.apple-touch-device .live-signal-pill,
    html.apple-touch-device .route-signal-icon,
    html.apple-touch-device .leaflet-marker-icon{pointer-events:auto!important;touch-action:manipulation!important;}
    ...
    /* iPad-specific layout: preserve desktop functionality, but make every control finger/Pencil friendly. */
    ...
    @media (min-width:700px) and (max-width:1180px){
      html.ipad-device body{padding-top:env(safe-area-inset-top);padding-bottom:env(safe-area-inset-bottom);padding-left:env(safe-area-inset-left);padding-right:env(safe-area-inset-right);box-sizing:border-box;}
      html.ipad-device #map,html.ipad-device .leaflet-container{min-height:420px;}
      html.ipad-device .leaflet-control-layers{font-size:15px;max-width:44vw;}
      html.ipad-device .leaflet-control-layers label{min-height:34px;display:flex;align-items:center;gap:7px;}
    }

    @media (orientation:landscape) and (min-width:700px){
      html.ipad-device .leaflet-control-container .leaflet-top{top:max(8px,env(safe-area-inset-top));}
      html.ipad-device .leaflet-control-container .leaflet-bottom{bottom:max(8px,env(safe-area-inset-bottom));}
      html.ipad-device .leaflet-control-container .leaflet-left{left:max(8px,env(safe-area-inset-left));}
      html.ipad-device .leaflet-control-container .leaflet-right{right:max(8px,env(safe-area-inset-right));}
    }
  `;
  document.head.appendChild(style);

  // Apple Pencil reports pointerType="pen". Keep it working exactly like a finger for custom controls.
  document.addEventListener('pointerup',e=>{
    if(e.pointerType!=='pen')return;
    const t=e.target&&e.target.closest?e.target.closest('button,[role="button"],a'):null;
    if(!t||t.disabled)return;
    // Native click usually follows pointerup. Only synthesize when WebKit fails to emit one.
    t.__lastPenPointerUp=Date.now();
    setTimeout(()=>{
      if(Date.now()-(t.__lastNativeClick||0)>350&&Date.now()-(t.__lastPenPointerUp||0)<500){
        try{t.click();}catch(_){}
      }
    },180);
  },true);
  ...
  function fire(e){
    const now=Date.now();if(now-lastFire<700)return;lastFire=now;
    if(e){e.preventDefault();e.stopPropagation();}
    targetBtn=findLocBtn();
    try{
      if(targetBtn&&typeof targetBtn.onclick==='function'){
        targetBtn.onclick.call(targetBtn,new MouseEvent('click',{bubbles:true,cancelable:true,view:window}));return;
      }
  ...
  setInterval(syncOverlay,400);setTimeout(syncOverlay,50);
})();